...
# Load image 
image = cv2.imread(r"C:\Users\DELL\Desktop\github_cv\Image_enhancement\image_enchancement.jpg")  # Replace with your image path 
if image is None: 
...
cv2.imshow("Grayscale", label_image(cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR), 
"Grayscale")) 
...
cv2.imshow("Histogram Equalization", label_image(cv2.cvtColor(equalized, 
cv2.COLOR_GRAY2BGR), "Histogram Equalization")) 
...
cv2.imshow("Sobel Combined", label_image(cv2.convertScaleAbs(sobel_combined), 
"Sobel Combined")) 
...
cv2.imshow("Canny Edges", label_image(cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR), 
